...
function Drawer({
  ...
  const contextValue = React.useMemo(
    () => ({
      hasSnapPoints,
      modal,
      showSwipeHandle: dismissible ? showSwipeHandle : false,
      swipeDirection,
    }),
    [hasSnapPoints, modal, dismissible, showSwipeHandle, swipeDirection],
  )

  return (
    <DrawerContext.Provider value={contextValue}>
      <DrawerPrimitive.Root
        // TODO: Investigate if data-base-ui-swipe-ignore works as expected.
        data-base-ui-swipe-ignore={!dismissible}
        data-slot="drawer"
        disablePointerDismissal={!dismissible}
        modal={dismissible ? modal : false}
...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-50 min-h-dvh select-none bg-black/10 opacity-[max(var(--drawer-overlay-min-opacity,0),calc(1-var(--drawer-swipe-progress)))] transition-opacity duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] data-ending-style:pointer-events-none data-ending-style:opacity-0 data-starting-style:opacity-0 data-ending-style:duration-[calc(var(--drawer-swipe-strength)*0.4s)] data-swiping:duration-0 supports-[-webkit-touch-callout:none]:absolute supports-backdrop-filter:backdrop-blur-xs data-snap-points:[--drawer-overlay-min-opacity:0.5]",
        className,
      )}
      ...
      >
        <DrawerPrimitive.Popup
          className={cn(
            ...
            // Transitions.
            "data-ending-style:transform-(--closed-transform) data-starting-style:transform-(--closed-transform) data-ending-style:data-nested-drawer-swiping:duration-[calc(var(--drawer-swipe-strength)*0.4s)] data-ending-style:data-swiping:duration-[calc(var(--drawer-swipe-strength)*0.4s)] data-ending-style:duration-[calc(var(--drawer-swipe-strength)*0.4s)] data-nested-drawer-swiping:duration-0 data-swiping:duration-0",
            // Axis: y.
